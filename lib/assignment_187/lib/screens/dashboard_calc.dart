...
class ComputeScreen extends StatefulWidget {
  ...
  @override State<ComputeScreen> createState() => _ComputeScreenState();
}
class _ComputeScreenState extends State<ComputeScreen> {
  late final ComputeController _ctrl;
  bool _deg = true;
  @override void initState() { super.initState(); _ctrl = ComputeController(); }
  @override void dispose()   { _ctrl.dispose(); super.dispose(); }

  static const _kBg=Color(0xFF000000);static const _kSci=Color(0xFF001A22);static const _kSciTx=Color(0xFF69FFEF);
  static const _kNum=Color(0xFF0A0A0A);static const _kOp=Color(0xFF003344);static const _kOpTx=Color(0xFF69FFEF);
  static const _kFn=Color(0xFF0A0A0A);static const _kEq=Color(0xFF00E5FF);static const _kAc=Color(0xFFB71C1C);
  Widget _key(String lbl, VoidCallback fn, {Color bg=_kNum,Color fg=Colors.white,int flex=1,double fs=15.5}) {
  ...
  Widget _scaffold(String d,String e,bool deg,VoidCallback tg){
    return Scaffold(backgroundColor:_kBg,body:SafeArea(child:Column(children:[
      Container(width:double.infinity,color:const Color(0xFFCDD8A8),padding:const EdgeInsets.fromLTRB(14,10,14,12),child:Column(crossAxisAlignment:CrossAxisAlignment.end,children:[
        ...
        Text(d,style:TextStyle(color:const Color(0xFF1A2A08),fontSize:44.0,fontWeight:FontWeight.w100,letterSpacing:-1.5),textAlign:TextAlign.right,maxLines:1,overflow:TextOverflow.ellipsis),
      ])),
      Expanded(child:Padding(padding:const EdgeInsets.all(3),child:Column(children:[
        ...
        _row([_key('asin', () => _ctrl.sci('asin'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('acos', () => _ctrl.sci('acos'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('atan', () => _ctrl.sci('atan'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('√x', () => _ctrl.sci('sqrt'), bg:_kSci, fg:_kSciTx), _key('x²', () => _ctrl.sci('x²'), bg:_kSci, fg:_kSciTx)]),
        _row([_key('sinh', () => _ctrl.sci('sinh'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('cosh', () => _ctrl.sci('cosh'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('tanh', () => _ctrl.sci('tanh'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('x³', () => _ctrl.sci('x³'), bg:_kSci, fg:_kSciTx), _key('1/x', () => _ctrl.sci('1/x'), bg:_kSci, fg:_kSciTx)]),
        _row([_key('n!', () => _ctrl.sci('n!'), bg:_kSci, fg:_kSciTx), _key('nPr', () => _ctrl.combo('nPr'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('nCr', () => _ctrl.combo('nCr'), bg:_kSci, fg:_kSciTx, fs:13.5), _key('π', () => _ctrl.sci('π'), bg:_kSci, fg:const Color(0xFFFFD54F)), _key('e', () => _ctrl.sci('e'), bg:_kSci, fg:const Color(0xFFFFD54F))]),
